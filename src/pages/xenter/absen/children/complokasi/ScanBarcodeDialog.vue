...
function onDecodeString (val) {
  console.log('ScanBarcodeDialog: QR Code diterima:', val)
  const formData = {
    qr: val,
  ...
  console.log('ScanBarcodeDialog: Mengirim data form:', formData)
  kirimQr.value = true
  waiting.value = true
  sendData(formData)
}
...
async function sendData (data) {
  console.log('ScanBarcodeDialog: Memanggil API untuk mengirim data QR...', data)
  try {
    await api.post('/v2/absensi/scan/qr', data)
    console.log('ScanBarcodeDialog: API berhasil mengirim data QR.')
    props.kondisi === 'masuk' ? saveStore('checkIn') : saveStore('checkOut')
  } catch (error) {
    console.error('ScanBarcodeDialog: API gagal mengirim data QR:', error)
    err.value = 'Ada Kesalahan Harap Ulangi'
  } finally {
    console.log('ScanBarcodeDialog: Proses pengiriman data selesai.')
    waiting.value = false
  }
}
</script>
